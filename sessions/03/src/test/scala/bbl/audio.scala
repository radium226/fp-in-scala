...
class AudioSuite extends AbstractSuite {
  ...
  test("Audio should be able to play") {
    val musicFilePath = Paths.get("sessions/03/src/test/resources/parisians.mp3")
    Audio.readFile(musicFilePath).play.unsafeRunSync()
  }
  test("Audio should be able to generate brown noise") {
    Audio.noise(NoiseColor.White)
      .take(1.second)
      .play
      .unsafeRunSync()
  }
  ...
  test("Audio can be concatenated") {
    Audio.noise(NoiseColor.Brown).take(1.second).concat(Audio.noise(NoiseColor.Pink).take(1.second))
      .play
      .unsafeRunSync()
  }
